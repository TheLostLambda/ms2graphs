...
class Bio_Graph:
    ...
    def construct_graph(self):
        ...
        master_graph = nx.from_pandas_edgelist(self.edges_from_file, 'node1', 'node2', edge_attr=True)

        # add node attributes from node list
        node_attributes = self.nodes_from_file.set_index('node').to_dict('index').items()
    ...
    def graph_hash(self, graph):
        ...
        node_ids = str([n[1]['molecule_ID'] for n in graph.nodes(data=True)])
        # List all edges as string values
        edges = str(graph.edges)
        # Generate graph ID by combining node and edge strings
        graph_id = node_ids + edges
        # Set hash algorithm
        h = hl.blake2b()
        # Generate hash value for graph (utf8 encoding)
        h.update(graph_id.encode('utf8'))
    ...
    def monoisotopic_mass_calculator(self, graph_fragments, graph_IDs):
        ...
        for f_graph in graph_IDs:
            ...
            node_molecule_ID = [n[1]['molecule_ID'] for n in g.nodes(data=True)]
    ...
    def bond_cutter(self, intact_graph):
        ...
        for (u, v) in intact_graph.edges:
                ...
                fragments = [cp.deepcopy(intact_copy.subgraph(comps)) for comps in nx.connected_components(intact_copy)]
    ...
    def fragmentation(self, molecule, cut_counter = 1, cut_limit = 1):
        # dict for all cut products
        all_fragments = {}
        # Dynamically creates a list of dictionaries based on cut limit (one dictionary for each cutting pass)
        cut_products = [dict() for x in range(cut_limit)]
        # Populates initial dictionary with all unique single cut fragments 
        cut_products[0] = self.bond_cutter(molecule)
        # Performes multiple cut passes on parent graph (each cut is sequential e.g 2nd pass is performed on the list of 1st cut fragments)
        for cut in range(cut_limit-1):
            #Loops through graph in given cut_products[cut] dictionary [cut] represents 1st,2nd,3rd cut dictionary. E.G cut_products[2] = dictionary containing all 2 cut fragments 
        ...
        for nth_product in cut_products:
            for graph_hash in nth_product:
                if graph_hash not in all_fragments:
                    graph = nx.Graph(nth_product[graph_hash])
                    all_fragments.update({graph_hash: graph})
    ...
    def generate_mass_to_charge_masses(self,n_terminal_fragments:list,c_terminal_fragments:list,internal_fragments:list, enabled_ions:list, charge_limit: int = 1):
        ...
        if 'x' in enabled_ions:
            for (mono_mass,graph) in c_terminal_fragments:
                for charge_state in range(1, charge_limit+1):
                    x_neutral = mono_mass[0] + C_term_neutral + keto_mass - hydrogen_mass
                    x_mz.append((x_neutral+(proton_mass*charge_state))/charge_state)
        ...
        if 'z' in enabled_ions:
            for (mono_mass,graph) in c_terminal_fragments:
                for charge_state in range(1, charge_limit+1):
                    ...
                    z_mz.append((z_neutral+(proton_mass*charge_state))/charge_state)
